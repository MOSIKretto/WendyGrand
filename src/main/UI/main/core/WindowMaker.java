...
public class WindowMaker extends JFrame 
{
    ...
    private void initUI() 
    {
        ...
        {
            @Override
            public void keyPressed(KeyEvent e) { if (e.getKeyCode() == KeyEvent.VK_SPACE) spacePressed = true; }
            
            @Override
            public void keyReleased(KeyEvent e) { if (e.getKeyCode() == KeyEvent.VK_SPACE) spacePressed = false; }
        });
        ...
        KeyboardFocusManager.getCurrentKeyboardFocusManager().addKeyEventDispatcher(e -> 
        {
            ...
            else if (e.getID() == KeyEvent.KEY_RELEASED) { if (e.getKeyCode() == KeyEvent.VK_SPACE) spacePressed = false; }
            return false;
        });
    }
    ...
    private void handleKeyPress(KeyEvent e) 
    {
        ...
        {
            if (e.getKeyCode() == KeyEvent.VK_Q) dispose();
            return;
        }
        ...
        switch (currentFocusState) 
        {
            case MAIN_MENU:
                if (e.getKeyCode() == KeyEvent.VK_RIGHT) 
                {
                    ...
                    switch (selected) 
                    {
                        case "Модули" -> focusOnPanel("Modules");
                        case "Словари" -> focusOnPanel("Dictionaries");
                        case "Настройки" -> focusOnPanel("Settings");
                    }
                }
                break;
    ...
    private void setupWindowDragHandlers(Component dragComponent) 
    {
        ...
        dragComponent.addMouseListener(new MouseAdapter() 
        {
            ...
            public void mouseReleased(MouseEvent e) { if (SwingUtilities.isLeftMouseButton(e)) isDragging = false; }
        });
    }

    public void navigateTo(int index) { ((CardLayout) contentPanel.getLayout()).show(contentPanel, String.valueOf(index)); }
    ...
    private void setFocus(FocusState newState) { currentFocusState = newState; }
    public void setCurrentFocusState(FocusState state) { this.currentFocusState = state; }
    public FocusState getCurrentFocusState() { return currentFocusState; }
}